...
begin = ['(100)', '(111)', '(112)', '(121)', '(122)', '(201)', '(202)',
         '(301)', '(302)', '(400)']
sheet0 = 0
op = ''
...
def transformation(event):
    ...
    if op:    
        if _date in day:
            ...
            tex.delete(1.0,END)
            tex.insert(END,'Число должно быть в диапазоне от 01 до' +
...
def _open():
    ...
    tex.insert(END, '.\nТеперь введите день, для которого будет выполнено' +
               'преобразование')
...
def _help():
     win = Toplevel(root)
     lab = Label(win, text="Вам необходимо ввести в поле день, отчет за" +
                 " какой необходимо преобразовать.\nЧисло необходимо вводить" +
                 " в следующем формате - '01, 02, 03 и т.д.'. После этого" +
                 " нажать на кнопку Преобразовать.\nЕсли вы правильно ввели" +
                 " число и файл в формате xls существует, то он будет" +
                 " преобразован в формат txt\nи результат преобразования" +
                 " появится на экране. Файл txt будет автоматически сохранен.")
     lab.pack()

def _about():
     win = Toplevel(root)
     lab = Label(win, text="Эта программа преобразовывает xls файл \n в txt" +
                 " формат специальной разметкой.")
     lab.pack()
...
lab1 = Label(fra1, text="Откройте xls файл, который хотите преобразовать\n" +
             ...
             " текстовый файл. Будьте внимательны\nвыбирайте файл и день" +
             " месяца одинаковые.\nС 1-го по 9-е числа вводятся как 01, 02" +
             " и т.д.", font="Arial 10")
lab2 = Label(fra3, text="Если введённое чило не верно, его можно сменить и" +
             " снова выполнить\nпреобразование кнопкой Transform, не" +
             " открывая файл второй раз\nДля сохранения результата выберите" +
             " File->Save.", font="Arial 10")
